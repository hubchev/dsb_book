	...
	As the reader has probably noticed, it is rather easy for us, humans, to infer the sentiment of a sentence, as we have been trained to understand human languages since the very first day we were born. However, the approached that we used, i.e. sequentially reading sentences and inferring their sentiments is not really scalable. What if we needed to infer the sentiment of millions of sentences in a short amount of time? We would lots of humans. This seems like a resource intensive task, and there are probably tools to help us. Computers are faster and can even compute task in concurrently. Therefore, the subsequent question that might arise in your mind is \emph{How to compute the sentiment of textual data?}
